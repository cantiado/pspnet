<template>
  <div class="flex flex-col  h-screen">
    <nav class="flex justify-start bg-plant h-24 items-center">
      ...
      <router-link v-if="!routeDisableNames.includes(currentRoute)" class = "mx-2" :to="{name: 'identify'}">Identify</router-link>
      <router-link v-if="!routeDisableNames.includes(currentRoute)" class = "mx-2" :to="{name: 'about'}">About</router-link> 
      <router-link v-if="!routeDisableNames.includes(currentRoute)" class = "mx-2" :to="{name: 'explore'}">Explore</router-link> 
