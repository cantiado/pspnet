  ...
  <div class="w-full flex flex-row flex-wrap">
    <div class="basis-1/3 flex flex-col">
      <form method="POST" action="" enctype="multipart/form-data">
        <div id="file-upload" class="flex items-center justify-center font-sans m-5">
          <label for="dropzone-file" class="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center rounded-xl border-2 border-dashed border-blue-400 bg-white p-6 text-center">
            ...
            <input @change="uploadImages" id="dropzone-file" type="file" accept="image/png, image/jpeg" class="hidden" />
          </label>
        </div>
        ...
        <div v-else-if="selectedModel=='None'" class="m-5 flex flex-col items-center">
          <button disabled class="w-5/6 h-10 bg-gray-200 rounded-full">
            <span class="text-xl font-bold">
              Identify
            </span>
    ...
    <div class="basis-2/3">
      <div v-if="uploadedImages" v-for="file in dropzoneFile.files">
        <span>{{ file.name }}</span>
      </div>
      <div v-else>
