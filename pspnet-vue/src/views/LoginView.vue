  ...
  <div class="w-full grid grid-cols-10">
    <div class="col-span-6 grid place-content-center bg-gray-50">
      ...
      <form @submit.prevent="handleLogin" class="form">
        ...
        <div class="flex justify-center">
          <router-link class="underline text-blue-500" :to="{ name: 'forgot' }"
        ...
        <div class="flex justify-center">
          <button
            ...
            class="bg-soil text-white hover:bg-pecan button"
          >
            Sign In
          </button>
          <button
            v-else
            class="bg-soil text-white button disabled:opacity-50 disabled"
          >
            Loading...
          </button>
        </div>
    ...
    <div class="bg-soil col-span-4 grid place-content-center">
      <div class="max-w-md">
        ...
        <p class="text-white my-6 text-center">
          Create an account to upload and contribute to the community!
        </p>
        <div class="flex justify-center">
          <button
            @click="handleCreate"
            class="bg-plant text-black button hover:bg-green-400"
          >
            Create Account
          </button>
        </div>
...
async function handleLogin() {
  error_msg.value = await store.loginUser({
    email: email.value,
    password: pass.value,
  });
  if (!error_msg.value) {
